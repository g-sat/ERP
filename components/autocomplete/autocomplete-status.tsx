...
import { useStatusLookup } from "@/hooks/use-lookup"
...
export default function StatusAutocomplete<T extends Record<string, unknown>>({
  ...
  const { data: statuss = [], isLoading } = useStatusLookup()
  // Memoize options to prevent unnecessary recalculations
  const options: FieldOption[] = React.useMemo(
    () =>
      statuss.map((status: IStatusLookup) => ({
  ...
  const handleChange = React.useCallback(
    ...
    [form, name, onChangeEvent, statuss]
  )
