...
import { IEmpCategoryLookup } from "@/interfaces/lookup"
...
import { useEmpCategoryLookup } from "@/hooks/use-lookup"
...
export default function EmployeeCategoryAutocomplete<
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IEmpCategoryLookup | null) => void
}) {
  const { data: empCategorys = [], isLoading } = useEmpCategoryLookup()
  // Memoize options to prevent unnecessary recalculations
  const options: FieldOption[] = React.useMemo(
    () =>
      empCategorys.map((empCategory: IEmpCategoryLookup) => ({
        value: empCategory.empCategoryId.toString(),
        label: empCategory.empCategoryName,
      })),
    [empCategorys]
  )
  ...
  const handleChange = React.useCallback(
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedEmpCategory = selectedOption
          ? empCategorys.find(
              (u: IEmpCategoryLookup) =>
                u.empCategoryId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedEmpCategory)
      }
    },
    [form, name, onChangeEvent, empCategorys]
  )
            ...
            return (
              <FormItem className={cn("flex flex-col", className)}>
                <Select
                  ...
                  placeholder="Select EmpCategory..."
                  ...
                  menuPosition="fixed"
                  isLoading={isLoading}
                  loadingMessage={() => "Loading empCategorys..."}
                />
                {showError && (
