...
export function AuthInitializer() {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && token) {
      try {
        console.log("🔐 [AuthInitializer] Validating token on app start...")

        const decoded = jwtDecode(token)

        if (!decoded || !decoded.exp) {
          console.log(
            "❌ [AuthInitializer] Invalid token structure, logging out"
          )
          logOutSuccess()
          router.push("/login")
          return
        ...
        if (isExpired) {
          console.log("❌ [AuthInitializer] Token expired, logging out")
          logOutSuccess()
          router.push("/login")
          return
        }
        ...
        if (expiresSoon) {
          console.log(
            "⚠️ [AuthInitializer] Token expires soon, will refresh automatically"
          )
          // The automatic refresh system will handle this
        }

        console.log("✅ [AuthInitializer] Token valid, continuing")
      } catch (error) {
        console.error(
      ...
      console.log(
        "🔒 [AuthInitializer] No authentication, redirecting to login"
      )
      router.push("/login")
    }
  }, [
