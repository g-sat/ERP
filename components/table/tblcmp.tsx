...
import { RefreshCw, Download, X, ChevronRight, ChevronLeft } from "lucide-react";
...
export default function InvoiceTable({
  ...
  useEffect(() => {
    ...
    setSearchStartDate(initialFilters?.startDate?.toString() || defaultStartDate);
  ...
  const mappedRows = useMemo(
    () =>
      data.map((invoice) => ({
        ...invoice,
        trnDateDisplay: invoice.trnDate ? format(new Date(invoice.trnDate), dateFormat) : "-",
        accountDateDisplay: invoice.accountDate ? format(new Date(invoice.accountDate), dateFormat) : "-",
        deliveryDateDisplay: invoice.deliveryDate ? format(new Date(invoice.deliveryDate), dateFormat) : "-",
        dueDateDisplay: invoice.dueDate ? format(new Date(invoice.dueDate), dateFormat) : "-",
        createDateDisplay: invoice.createDate ? format(new Date(invoice.createDate), dateFormat) : "-",
        editDateDisplay: invoice.editDate ? format(new Date(invoice.editDate), dateFormat) : "-",
        ...
        totLocalAmtAftGstDisplay: formatNumber(invoice.totLocalAmtAftGst ?? 0, locAmtDec),
        balAmtDisplay: formatNumber(invoice.balAmt ?? 0, amtDec),
        exhRateDisplay: formatNumber(invoice.exhRate ?? 0, decimals[0]?.exhRateDec || 9),
        ctyExhRateDisplay: formatNumber(invoice.ctyExhRate ?? 0, decimals[0]?.exhRateDec || 9),
        paymentStatusDisplay: getPaymentStatus(invoice.balAmt, invoice.payAmt, invoice.isCancel ?? false),
      })),
  ...
  (
    () => [
      { accessorKey: "invoiceNo", header: "Invoice No", sortable: true, size: 140, minSize: 120, maxSize: 200 },
      {
          ...
          return (
            <div className="flex justify-center">
              <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-medium ${badgeClass}`}>
                <span className={`mr-1 h-2 w-2 rounded-full ${dotClass}`}></span>
                {status || "Cancelled"}
              </span>
            </div>
          );
      ...
      { accessorKey: "referenceNo", header: "Reference No", size: 160, minSize: 140, maxSize: 240 },
      { accessorKey: "trnDateDisplay", header: "Transaction Date", size: 160, minSize: 140, maxSize: 220 },
      { accessorKey: "accountDateDisplay", header: "Account Date", size: 160, minSize: 140, maxSize: 220 },
      { accessorKey: "deliveryDateDisplay", header: "Delivery Date", size: 160, minSize: 140, maxSize: 220 },
      { accessorKey: "dueDateDisplay", header: "Due Date", size: 160, minSize: 140, maxSize: 220 },
      { accessorKey: "supplierCode", header: "Supplier Code", size: 160, minSize: 140, maxSize: 220 },
      { accessorKey: "supplierName", header: "Supplier Name", size: 220, minSize: 160, maxSize: 320 },
      { accessorKey: "currencyCode", header: "Currency Code", size: 140, minSize: 120, maxSize: 200 },
      { accessorKey: "currencyName", header: "Currency Name", size: 200, minSize: 160, maxSize: 280 },
      { accessorKey: "exhRateDisplay", header: "Exchange Rate", align: "right", size: 160, minSize: 140, maxSize: 220 },
      {
      ...
      { accessorKey: "creditTermCode", header: "Credit Term Code", size: 180, minSize: 150, maxSize: 240 },
      { accessorKey: "creditTermName", header: "Credit Term Name", size: 220, minSize: 180, maxSize: 320 },
      { accessorKey: "bankCode", header: "Bank Code", size: 140, minSize: 120, maxSize: 200 },
      { accessorKey: "bankName", header: "Bank Name", size: 200, minSize: 160, maxSize: 280 },
      { accessorKey: "totAmtDisplay", header: "Total Amount", align: "right", size: 160, minSize: 140, maxSize: 220 },
      {
      ...
      { accessorKey: "gstAmtDisplay", header: "GST Amount", align: "right", size: 140, minSize: 120, maxSize: 200 },
      {
      ...
      { accessorKey: "status", header: "Status", size: 140, minSize: 120, maxSize: 200 },
      { accessorKey: "createBy", header: "Created By", size: 220, minSize: 180, maxSize: 320 },
      { accessorKey: "createDateDisplay", header: "Created Date", size: 180, minSize: 150, maxSize: 240 },
      { accessorKey: "editBy", header: "Edited By", size: 220, minSize: 180, maxSize: 320 },
      { accessorKey: "editDateDisplay", header: "Edited Date", size: 180, minSize: 150, maxSize: 240 },
      { accessorKey: "editVersion", header: "Edit Version", size: 140, minSize: 120, maxSize: 200 },
    ],
    []
  );
  ...
  return (
    <div className="w-full overflow-auto">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                ...
                <CustomDateNew form={form} name="startDate" isRequired size="sm" />
              </div>
              <div className="flex items-center gap-2">
                <span className="text-muted-foreground text-sm font-medium">To:</span>
                <CustomDateNew form={form} name="endDate" isRequired size="sm" />
              </div>
            </div>
            <Button variant="default" size="sm" onClick={handleSearchInvoice} disabled={isLoading}>
              {isLoading ? (
                <>
      ...
      {isLoading && <div className="mb-3 text-sm text-muted-foreground">Loading invoices...</div>}
