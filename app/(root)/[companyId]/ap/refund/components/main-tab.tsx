...
import PaymentDetailsTable from "./refund-details-table"
import PaymentForm from "./refund-form"
...
export default function Main({
  ...
  useEffect(() => {
    if (dataDetails.length > 0) {
      const totalExhGainLoss = calculateTotalExchangeGainLoss(
        dataDetails as unknown as IApRefundDt[],
        decimals[0] || {
          amtDec: 2,
          locAmtDec: 2,
          ctyAmtDec: 2,
  ...
  // Watch currency and refund total fields
  ...
  const validateAllocation = useCallback((data: ApRefundDtSchemaType[]) => {
    if (data.length === 0) {
      toast.warning("No refund details to allocate")
      return false
    }
    return true
  }, [])
  ...
  const calculateItemAllocation = useCallback(
    (item: ApRefundDtSchemaType, allocAmt: number) => {
  ...
  const updateHeaderAmounts = useCallback(
    (data: ApRefundDtSchemaType[], totAmt?: number, totLocalAmt?: number) => {
      ...
      const totalExhGainLoss = calculateTotalExchangeGainLoss(
        data as unknown as IApRefundDt[],
        decimals[0] || {
          amtDec: 2,
          locAmtDec: 2,
          ctyAmtDec: 2,
  ...
  const calculateAmountSums = useCallback((data: ApRefundDtSchemaType[]) => {
  ...
  const handleResetAllocation = useCallback(() => {
    ...
    if (currentData.length === 0) {
      toast.warning("No refund details to reset")
      return
    }
  ...
  const handleSelectTransaction = useCallback(() => {
    ...
    if (!supplierId || !currencyId || !accountDate || !paymentTypeId) {
      toast.warning(
        "Please select Supplier, Currency, Account Date and Refund Type first"
      )
      return
    }
    ...
    dialogParamsRef.current = {
      ...
      isRefund: true,
      documentId: form.getValues("refundId")?.toString() || "",
