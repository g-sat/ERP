...
export default function InvoicePage() {
  ...
  const [invoice, setInvoice] = useState<ApCreditNoteHdSchemaType | null>(null)
  ...
  const form = useForm<ApCreditNoteHdSchemaType>({
    ...
    defaultValues: invoice
      ? {
          creditNoteId: invoice.creditNoteId?.toString() ?? "0",
          creditNoteNo: invoice.creditNoteNo ?? "",
          ...
          suppCreditNoteNo: invoice.suppCreditNoteNo ?? "",
  ...
  } = useGetWithDates<IApCreditNoteHd>(
    `${ApCreditNote.get}`,
    TableName.apCreditNote,
    filters.search,
  ...
  const invoicesData = useMemo(
    () => (invoicesResponse as ApiResponse<IApCreditNoteHd>)?.data ?? [],
    [invoicesResponse]
  )
      ...
      if (response.result === 1) {
        ...
        if (invoiceData) {
          const updatedSchemaType = transformToSchemaType(
            invoiceData as unknown as IApCreditNoteHd
          )
        ...
        const wasNewInvoice = Number(formValues.creditNoteId) === 0

        if (wasNewInvoice) {
          //toast.success(
          // `Invoice ${invoiceData?.creditNoteNo || ""} saved successfully`
          //)
        } else {
  ...
  const handleCloneInvoice = () => {
    if (invoice) {
      ...
      const clonedInvoice: ApCreditNoteHdSchemaType = {
        ...invoice,
        creditNoteId: "0",
        creditNoteNo: "",
  ...
  const transformToSchemaType = (
    apiInvoice: IApCreditNoteHd
  ): ApCreditNoteHdSchemaType => {
    return {
      creditNoteId: apiInvoice.creditNoteId?.toString() ?? "0",
      creditNoteNo: apiInvoice.creditNoteNo ?? "",
      referenceNo: apiInvoice.referenceNo ?? "",
      suppCreditNoteNo: apiInvoice.suppCreditNoteNo ?? "",
      ...
      addressId: apiInvoice.addressId ?? 0, // Not available in IApCreditNoteHd
      contactId: apiInvoice.contactId ?? 0, // Not available in IApCreditNoteHd
  ...
  const handleInvoiceSelect = async (
    selectedInvoice: IApCreditNoteHd | undefined
  ) => {
    ...
    try {
      ...
      const response = await getById(
        `${ApCreditNote.getByIdNo}/${selectedInvoice.creditNoteId}/${selectedInvoice.creditNoteNo}`
      )
      ...
      if (response?.result === 1) {
        ...
        if (detailedInvoice) {
          ...
          const updatedInvoice = {
            ...detailedInvoice,
            creditNoteId: detailedInvoice.creditNoteId?.toString() ?? "0",
            creditNoteNo: detailedInvoice.creditNoteNo ?? "",
            referenceNo: detailedInvoice.referenceNo ?? "",
            suppCreditNoteNo: detailedInvoice.suppCreditNoteNo ?? "",
            ...
            addressId: detailedInvoice.addressId ?? 0, // Not available in IApCreditNoteHd
            contactId: detailedInvoice.contactId ?? 0, // Not available in IApCreditNoteHd
            ...
            data_details:
              detailedInvoice.data_details?.map((detail: IApCreditNoteDt) => ({
                creditNoteId: detail.creditNoteId?.toString() ?? "0",
                creditNoteNo: detail.creditNoteNo ?? "",
                itemNo: detail.itemNo ?? 0,
                seqNo: detail.seqNo ?? 0,
          ...
          //setInvoice(updatedInvoice as ApCreditNoteHdSchemaType)
          ...
          toast.success(
            `Invoice ${selectedInvoice.creditNoteNo} loaded successfully`
          )
        }
      } else {
  ...
  return (
    <div className="@container flex flex-1 flex-col p-4">
        ...
        <div className="mb-2 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Input
              ...
              readOnly={!!invoice?.creditNoteId && invoice.creditNoteId !== "0"}
              disabled={!!invoice?.creditNoteId && invoice.creditNoteId !== "0"}
            />
            <Button
              variant="outline"
              size="sm"
            ...
            <Button
              variant="outline"
              size="sm"
              disabled={!invoice || invoice.creditNoteId === "0"}
            >
      ...
      <SaveConfirmation
        ...
        itemName={invoice?.creditNoteNo || "New Invoice"}
        operationType={
          invoice?.creditNoteId && invoice.creditNoteId !== "0"
            ? "update"
            : "create"
        }
        isSaving={
