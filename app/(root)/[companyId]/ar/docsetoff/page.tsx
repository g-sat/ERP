...
export default function DocSetOffPage() {
        ...
        if (response.result === 1) {
          ...
          if (receiptData) {
            const updatedSchemaType = transformToSchemaType(
              receiptData as unknown as IArDocSetOffHd
            )
  ...
  const transformToSchemaType = useCallback(
    (apiReceipt: IArDocSetOffHd): ArDocSetOffHdSchemaType => {
      return {
      ...
      if (response?.result === 1) {
        ...
        if (detailedReceipt) {
          ...
          const updatedReceipt = {
            ...
            data_details:
              detailedReceipt.data_details?.map((detail: IArDocSetOffDt) => ({
                setoffId: detail.setoffId?.toString() ?? "0",
                setoffNo: detail.setoffNo ?? "",
                itemNo: detail.itemNo ?? 0,
                transactionId: detail.transactionId ?? 0,
          ...
          //setReceipt(updatedReceipt as ArDocSetOffHdSchemaType)
  ...
  const handleReceiptSearch = useCallback(
    async (value: string) => {
      ...
      try {
        const response = await getById(`${ArDocSetOff.getByIdNo}/0/${value}`)
