...
export function ConsignmentImportTab({
  ...
  // Mutations
  const saveMutation = usePersist<ConsignmentImportFormValues>(
    `${JobOrder_ConsignmentImport.add}`
  )
  const updateMutation = usePersist<ConsignmentImportFormValues>(
    `${JobOrder_ConsignmentImport.add}`
  )
    ...
    async (consignmentImportId: string, debitNoteNo?: string) => {
      try {
        ...
        if (response.result > 0) {
          ...
          const debitNoteResponse = (await getData(
            `${JobOrder_DebitNote.getById}/${jobData.jobOrderId}/${Task.ConsignmentImport}/${response.result}`
          )) as ApiResponse<IDebitNoteHd>
    ...
    if (debitNoteDeleteConfirmation.debitNoteId) {
      try {
        await debitNoteDeleteMutation.mutateAsync(
          `${jobData.jobOrderId}/${Task.ConsignmentImport}/${debitNoteDeleteConfirmation.debitNoteId}`
        )
        queryClient.invalidateQueries({ queryKey: ["consignmentImport"] })
      ...
      <div className="space-y-4">
        <div className="overflow-x-auto">
          <ConsignmentImportTable
            ...
            onCreateConsignmentImport={handleCreate}
      ...
      <SaveConfirmation
        ...
        itemName={
          saveConfirmation.operationType === "update"
            ? `Consignment Import ${selectedItem?.awbNo || ""}`
