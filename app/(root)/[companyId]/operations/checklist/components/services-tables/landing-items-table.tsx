...
export function LandingItemsTable({
  ...
  console.log(isConfirmed, "isConfirmed landing items")
  ...
  const selectedRowIds = useMemo(() => {
    ...
    return selectedIndices.map((index) =>
      data[parseInt(index)].landingItemId.toString()
    )
  }, [hasSelectedRows, data, rowSelection])
  ...
  const handleSearch = useCallback(
    (query: string) => {
      ...
      if (data && data.length > 0) {
        ...
        const newFilters: ILandingItemsFilter = {
          search: query,
  ...
  useEffect(() => {
    if (!data?.length && onFilterChange) {
      const filters: ILandingItemsFilter = {
        search: searchQuery,
        sortOrder: sorting[0]?.desc ? "desc" : "asc",
      }
  ...
  return (
    ...
    >
      <TableHeaderProject
        ...
        tableName={TableName.landing_items}
        moduleId={moduleId || 1}
