...
export function AgencyRemunerationTable({
  ...
  console.log(isConfirmed, "isConfirmed agency remuneration")
  ...
  const selectedRowIds = useMemo(() => {
    ...
    return selectedIndices.map((index) =>
      data[parseInt(index)].agencyRemunerationId.toString()
    )
  }, [hasSelectedRows, data, rowSelection])
  ...
  const handleSearch = useCallback(
    (query: string) => {
      ...
      if (data && data.length > 0) {
        ...
        const newFilters: IAgencyRemunerationFilter = {
          search: query,
  ...
  useEffect(() => {
    if (!data?.length && onFilterChange) {
      const filters: IAgencyRemunerationFilter = {
        search: searchQuery,
        sortOrder: sorting[0]?.desc ? "desc" : "asc",
      }
  ...
  return (
    ...
    >
      <TableHeaderProject
        ...
        tableName={TableName.agency_remuneration}
        moduleId={moduleId || 1}
