...
export function ThirdPartyTable({
  ...
  console.log(isConfirmed, "isConfirmed third party")
  ...
  const selectedRowIds = useMemo(() => {
    ...
    return selectedIndices.map((index) =>
      data[parseInt(index)].thirdPartyId.toString()
    )
  }, [hasSelectedRows, data, rowSelection])
  ...
  const handleSearch = useCallback(
    (query: string) => {
      ...
      if (data && data.length > 0) {
        ...
        const newFilters: IThirdPartyFilter = {
          search: query,
  ...
  useEffect(() => {
    if (!data?.length && onFilterChange) {
      const filters: IThirdPartyFilter = {
        search: searchQuery,
        sortOrder: sorting[0]?.desc ? "desc" : "asc",
      }
  ...
  return (
    ...
    >
      <TableHeaderProject
        ...
        tableName={TableName.third_party}
        moduleId={moduleId || 1}
