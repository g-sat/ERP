...
import { useGetByParams, useSave } from "@/hooks/use-common-v1"
...
export function CopyRateForm({ onClose }: CopyRateFormProps) {
  ...
  const copyTariffMutation = useSave(Tariff.copy, "tariffs", companyId)
  ...
  const apiKey = useMemo(() => {
    if (
      ...
      return `copyRateTariffs-${watchedFromCustomerId}-${watchedFromPortId}-${watchedFromTaskId}`
    }
    return null
