...
interface PortExpensesTableProps {
  ...
  onDebitNote?: (portExpenseId: string) => void
...
export function PortExpensesTable({
  ...
  console.log(isConfirmed, "isConfirmed port expenses")
  ...
  const selectedRowIds = useMemo(() => {
    ...
    return selectedIndices.map((index) =>
      data[parseInt(index)].portExpenseId.toString()
    )
  }, [hasSelectedRows, data, rowSelection])
    ...
    () => [
      {
        ...
        maxSize: 130,
        cell: ({ row }) => {
          const portExpenses = row.original
          const isSelected = row.getIsSelected()

          return (
            <TableActionsProject
              row={portExpenses}
              idAccessor="portExpenseId"
              onView={onPortExpensesSelect}
  ...
  const handleSearch = useCallback(
    (query: string) => {
      ...
      if (data && data.length > 0) {
        ...
        const newFilters: IPortExpensesFilter = {
          search: query,
  ...
  useEffect(() => {
    if (!data?.length && onFilterChange) {
      const filters: IPortExpensesFilter = {
        search: searchQuery,
        sortOrder: sorting[0]?.desc ? "desc" : "asc",
      }
              ...
              {virtualRows.length > 0 ? (
                ...
                <>
                  <TableRow>
                    ...
                    >
                      {isLoading ? "Loading..." : "No port expenses found."}
                    </TableCell>
                  </TableRow>
