...
interface ConsignmentExportTableProps {
  ...
  onDebitNote?: (consignmentExportId: string) => void
...
export function ConsignmentExportTable({
  ...
  const selectedRowIds = useMemo(() => {
    ...
    return selectedIndices.map((index) =>
      data[parseInt(index)].consignmentExportId.toString()
    )
  }, [hasSelectedRows, data, rowSelection])
        ...
        cell: ({ row }) => {
          ...
          return (
            <TableActionsProject
              row={{
                ...consignmentExport,
                debitNoteId: consignmentExport.debitNoteId || undefined,
              }}
              idAccessor="consignmentExportId"
              onView={onConsignmentExportSelect}
  ...
  const handleSearch = useCallback(
    (query: string) => {
      ...
      if (data && data.length > 0) {
        ...
        const newFilters: IConsignmentExportFilter = {
          search: query,
  ...
  useEffect(() => {
    if (!data?.length && onFilterChange) {
      const filters: IConsignmentExportFilter = {
        search: searchQuery,
        sortOrder: sorting[0]?.desc ? "desc" : "asc",
      }
