...
export function LaunchServiceTable({
  ...
  console.log(isConfirmed, "isConfirmed launch service")
  ...
  const selectedRowIds = useMemo(() => {
    ...
    return selectedIndices.map((index) =>
      data[parseInt(index)].launchServiceId.toString()
    )
  }, [hasSelectedRows, data, rowSelection])
    ...
    () => [
      {
        ...
        maxSize: 130,
        cell: ({ row }) => {
          const launchService = row.original
          const isSelected = row.getIsSelected()

          return (
            <TableActionsProject
              row={launchService}
              idAccessor="launchServiceId"
  ...
  const handleSearch = useCallback(
    (query: string) => {
      ...
      if (data && data.length > 0) {
        ...
        const newFilters: ILaunchServiceFilter = {
          search: query,
  ...
  useEffect(() => {
    if (!data?.length && onFilterChange) {
      const filters: ILaunchServiceFilter = {
        search: searchQuery,
        sortOrder: sorting[0]?.desc ? "desc" : "asc",
      }
  ...
  return (
    ...
    >
      <TableHeaderProject
        ...
        tableName={TableName.launch_service}
        moduleId={moduleId || 1}
