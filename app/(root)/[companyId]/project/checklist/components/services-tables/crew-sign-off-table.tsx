...
export function CrewSignOffTable({
    ...
    () => [
      {
        ...
        maxSize: 150,
        cell: ({ row }) => {
          const crewSignOff = row.original
          const isSelected = row.getIsSelected()
          return (
            <TableActionsProject
              row={crewSignOff}
              idAccessor="crewSignOffId"
              onView={onCrewSignOffSelect}
  ...
  const hasValidDebitNoteIds = useMemo(() => {
    return (
      ...
      selectedRows.some((row) => {
        const debitNoteId = (row.original as ICrewSignOff).debitNoteId
  ...
  const handleCombinedService = useCallback(() => {
    ...
    if (onCombinedService) {
      onCombinedService(
        selectedRows.map((row) => row.original.crewSignOffId.toString())
      )
    }
  ...
  const handleDebitNote = useCallback(() => {
    ...
    if (onDebitNote) {
      selectedRows.forEach((row) => {
        const crewSignOff = row.original
        onDebitNote(crewSignOff.crewSignOffId.toString())
      })
    }
  ...
  const handleSearch = useCallback(
    (query: string) => {
      ...
      if (data && data.length > 0) {
        ...
        const newFilters: ICrewSignOffFilter = {
          search: query,
  ...
  useEffect(() => {
    if (!data?.length && onFilterChange) {
      const filters: ICrewSignOffFilter = {
        search: searchQuery,
        sortOrder: sorting[0]?.desc ? "desc" : "asc",
      }
  ...
  return (
    ...
    >
      <TableHeaderProject
        ...
        tableName={TableName.crew_sign_off}
        moduleId={moduleId || 1}
