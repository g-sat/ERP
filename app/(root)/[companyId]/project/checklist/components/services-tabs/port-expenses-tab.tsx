...
export function PortExpensesTab({
  ...
  // Mutations
  const saveMutation = useSave<PortExpensesFormValues>(
    `${JobOrder_PortExpenses.add}`,
    "portExpenses",
    companyId
  )
  const updateMutation = useUpdate<PortExpensesFormValues>(
    `${JobOrder_PortExpenses.add}`,
    "portExpenses",
    companyId
  )
  const deleteMutation = useDelete(
    `${JobOrder_PortExpenses.delete}`,
    "portExpenses",
    companyId
  )
  ...
  const handleSelect = useCallback(
    async (item: IPortExpenses | undefined) => {
      ...
      try {
        const response = await apiProxy.get<ApiResponse<IPortExpenses>>(
  ...
  const handleDebitNote = useCallback(
    async (portExpenseId: string) => {
      // First, find and set the selected item
      const foundItem = data?.find(
        (item) => item.portExpenseId.toString() === portExpenseId
      )
      ...
      <Dialog open={showDebitNoteModal} onOpenChange={setShowDebitNoteModal}>
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription>
              Manage debit note details for this port expenses.
            </DialogDescription>
          </DialogHeader>
          <DebitNote
            jobData={jobData}
      ...
      <Dialog open={showPurchaseModal} onOpenChange={setShowPurchaseModal}>
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription>
              Manage purchase details for this port expenses.
            </DialogDescription>
          </DialogHeader>
          <PurchaseTable />
        </DialogContent>
