...
import { IDebitNoteHd, IJobOrderHd, IThirdParty } from "@/interfaces/checklist"
...
export function ThirdPartyTab({
  ...
  const handleSelect = useCallback(
    async (item: IThirdParty | undefined) => {
      ...
      try {
        const response = await apiProxy.get<ApiResponse<IThirdParty>>(
  ...
  const handleDebitNote = useCallback(
    async (thirdPartyId: string) => {
      // First, find and set the selected item
      const foundItem = data?.find(
        (item) => item.thirdPartyId.toString() === thirdPartyId
      )
      ...
      if (foundItem?.debitNoteId) {
        const debitNoteResponse = await apiProxy.get<ApiResponse<IDebitNoteHd>>(
          `${JobOrder_DebitNote.getById}/${jobData.jobOrderId}/${Task.ThirdParty}/${foundItem.debitNoteId}`
        )
        if (
      ...
      <Dialog open={showDebitNoteModal} onOpenChange={setShowDebitNoteModal}>
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription>
              Manage debit note details for this third party.
            </DialogDescription>
          </DialogHeader>
          <DebitNote
            jobData={jobData}
      ...
      <Dialog open={showPurchaseModal} onOpenChange={setShowPurchaseModal}>
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription>
              Manage purchase details for this third party.
            </DialogDescription>
          </DialogHeader>
          <PurchaseTable />
        </DialogContent>
