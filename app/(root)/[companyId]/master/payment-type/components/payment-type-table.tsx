...
interface PaymentTypesTableProps {
  data: IPaymentType[]
  isLoading?: boolean
  onPaymentTypeSelect?: (paymentType: IPaymentType | null) => void
  onDeletePaymentType?: (paymentTypeId: string) => void
  onEditPaymentType?: (paymentType: IPaymentType) => void
...
export function PaymentTypesTable({
  ...
  const columns: ColumnDef<IPaymentType>[] = [
    {
      ...
      cell: ({ row }) => (
        <TableActions
          row={row.original}
          idAccessor="paymentTypeId"
          onView={onPaymentTypeSelect}
          onEdit={onEditPaymentType}
  ...
  const handleSearch = (query: string) => {
    ...
    if (data && data.length > 0) {
      ...
      const newFilters: IPaymentTypeFilter = {
        search: query,
  ...
  useEffect(() => {
    if (!data?.length && onFilterChange) {
      const filters: IPaymentTypeFilter = {
        search: searchQuery,
        sortOrder: sorting[0]?.desc ? "desc" : "asc",
      }
