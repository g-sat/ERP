...
interface CountriesTableProps {
  ...
  onRefresh?: () => void
  onFilterChange?: (filters: ICountryFilter) => void
  moduleId?: number
...
export function CountriesTable({
  ...
  const columns: ColumnDef<ICountry>[] = [
    {
      ...
      cell: ({ row }) => (
        <TableActions
          row={row.original}
          idAccessor="countryId"
          onView={onCountrySelect}
    ...
    },
    {
      ...
      header: "Code",
      cell: ({ row }) => (
        <div className="font-medium">{row.getValue("countryCode")}</div>
      ),
      size: 120,
      minSize: 50,
      maxSize: 150,
  ...
  const handleSearch = (query: string) => {
    ...
    if (data && data.length > 0) {
      ...
      const newFilters: ICountryFilter = {
        search: query,
  ...
  return (
    <div>
      <TableHeader
        ...
        moduleId={moduleId || 1}
        transactionId={transactionId || MasterTransactionId.country}
        companyId={companyId}
      />
