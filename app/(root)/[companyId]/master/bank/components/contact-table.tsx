...
interface ContactsTableProps {
  data: IBankContact[]
  isLoading?: boolean
  onContactSelect?: (contact: IBankContact | undefined) => void
  onDeleteContact?: (contactId: string) => Promise<void>
  onEditContact?: (contact: IBankContact | undefined) => void
...
export function ContactsTable({
  ...
  useEffect(() => {
    if (!data?.length && onFilterChange) {
      const filters: IBankContactFilter = {
        search: searchQuery,
        sortOrder: sorting[0]?.desc ? "desc" : "asc",
      }
  ...
  return (
    <div>
      <TableHeader
        ...
        data={data}
        tableName={TableName.bank_contact}
