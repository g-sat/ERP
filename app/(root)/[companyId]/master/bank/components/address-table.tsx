...
interface AddresssTableProps {
  data: IBankAddress[]
  isLoading?: boolean
  onAddressSelect?: (address: IBankAddress | undefined) => void
  onDeleteAddress?: (addressId: string) => Promise<void>
  onEditAddress?: (address: IBankAddress | undefined) => void
...
export function AddresssTable({
  ...
  useEffect(() => {
    if (!data?.length && onFilterChange) {
      const filters: IBankAddressFilter = {
        search: searchQuery,
        sortOrder: sorting[0]?.desc ? "desc" : "asc",
      }
  ...
  return (
    <div>
      <TableHeader
        ...
        data={data}
        tableName={TableName.bank_address}
        hideCreateButton={false}
        moduleId={moduleId || ModuleId.master}
        transactionId={transactionId || MasterTransactionId.bank}
      />
