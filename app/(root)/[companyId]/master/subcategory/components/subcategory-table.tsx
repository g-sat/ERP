...
interface SubCategorysTableProps {
  ...
  onRefresh?: () => void
  onFilterChange?: (filters: ISubCategoryFilter) => void
  moduleId?: number
...
export function SubCategorysTable({
  ...
  const columns: ColumnDef<ISubCategory>[] = [
    {
      ...
      cell: ({ row }) => (
        <TableActions
          row={row.original}
          idAccessor="subcategoryId"
          onView={onSubCategorySelect}
    ...
    },
    {
      ...
      header: "Code",
      cell: ({ row }) => (
        <div className="font-medium">{row.getValue("subcategoryCode")}</div>
      ),
      size: 120,
      minSize: 50,
      maxSize: 150,
  ...
  const handleSearch = (query: string) => {
    ...
    if (data && data.length > 0) {
      ...
      const newFilters: ISubCategoryFilter = {
        search: query,
  ...
  return (
    <div>
      <TableHeader
        ...
        moduleId={moduleId || 1}
        transactionId={transactionId || MasterTransactionId.sub_category}
        companyId={companyId}
      />
