...
interface VesselsTableProps {
  data: IVessel[]
  isLoading?: boolean
  onVesselSelect?: (vessel: IVessel | null) => void
  onDeleteVessel?: (vesselId: string) => void
  onEditVessel?: (vessel: IVessel) => void
  onCreateVessel?: () => void
  onRefresh?: () => void
...
export function VesselsTable({
  ...
  const columns: ColumnDef<IVessel>[] = [
    ...
    },
    {
      ...
      header: "Code",
      cell: ({ row }) => (
        <div className="font-medium">{row.getValue("vesselCode")}</div>
      ),
      size: 120,
      minSize: 50,
      maxSize: 150,
  ...
  const handleSearch = (query: string) => {
    ...
    if (data && data.length > 0) {
      ...
      const newFilters: IVesselFilter = {
        search: query,
