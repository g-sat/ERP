...
interface BargesTableProps {
  data: IBarge[]
  isLoading?: boolean
  onBargeSelect?: (barge: IBarge | null) => void
  onDeleteBarge?: (bargeId: string) => void
  onEditBarge?: (barge: IBarge) => void
  onCreateBarge?: () => void
  onRefresh?: () => void
  onFilterChange?: (filters: IBargeFilter) => void
  moduleId?: number
  ...
  const columns: ColumnDef<IBarge>[] = [
    {
      ...
      cell: ({ row }) => (
        <TableActions
          row={row.original}
          idAccessor="bargeId"
          onView={onBargeSelect}
          onEdit={onEditBarge}
          onDelete={onDeleteBarge}
        />
      ),
    },
    {
    ...
    },
    {
      ...
      header: "Own Status",
      cell: ({ row }) => (
        <Badge variant={row.getValue("isOwn") ? "default" : "secondary"}>
          {row.getValue("isOwn") ? (
            <IconCircleCheckFilled className="mr-1 fill-green-500 dark:fill-green-400" />
          ) : (
            <IconSquareRoundedXFilled className="mr-1 fill-red-500 dark:fill-red-400" />
  ...
  return (
    <div>
      <TableHeader
        ...
        onRefresh={onRefresh}
        onCreate={onCreateBarge}
        columns={table.getAllLeafColumns()}
        data={data}
        tableName={TableName.barge}
        hideCreateButton={false}
        moduleId={moduleId || ModuleId.master}
        transactionId={transactionId || MasterTransactionId.barge}
        companyId={companyId}
      />
