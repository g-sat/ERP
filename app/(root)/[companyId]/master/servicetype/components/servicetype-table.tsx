...
import { IServiceType, IServiceTypeFilter } from "@/interfaces/servicetype"
...
interface ServiceTypeTableProps {
  data: IServiceType[]
  isLoading?: boolean
  onServiceTypeSelect?: (servicetype: IServiceType | undefined) => void
  onDeleteServiceType?: (servicetypeId: string) => void
  ...
  const columns: ColumnDef<IServiceType>[] = [
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <TableActions
            row={servicetype}
            idAccessor="serviceTypeId"
            onView={onServiceTypeSelect}
    ...
    },
    {
      ...
      header: "Code",
      cell: ({ row }) => (
        <div className="font-medium">{row.getValue("servicetypeCode")}</div>
      ),
      size: 120,
      minSize: 50,
      maxSize: 150,
...

    {
      ...
      cell: ({ row }) => (
        <div className="font-medium">
          {row.getValue("servicetypeCategoryName")}
        </div>
      ),
      size: 120,
      minSize: 50,
  ...
  return (
    <div>
      <TableHeader
        ...
        onRefresh={onRefresh}
        onCreate={onCreateServiceType}
        columns={table.getAllLeafColumns()}
        data={data}
        tableName={TableName.service_type}
        hideCreateButton={false}
        moduleId={moduleId || 1}
        transactionId={transactionId || MasterTransactionId.service_type}
        companyId={companyId}
      />
