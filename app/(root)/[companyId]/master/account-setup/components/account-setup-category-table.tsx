...
interface AccountSetupCategoryTableProps {
  ...
  onAccountSetupCategorySelect?: (
    accountSetupCategory: IAccountSetupCategory | null
  ) => void
  onDeleteAccountSetupCategory?: (accountSetupCategoryId: string) => void
  onEditAccountSetupCategory?: (
    accountSetupCategory: IAccountSetupCategory
  ) => void
  onCreateAccountSetupCategory?: () => void
  onRefresh?: () => void
...
export function AccountSetupCategoryTable({
  ...
  const columns: ColumnDef<IAccountSetupCategory>[] = [
    {
      ...
      cell: ({ row }) => (
        <TableActions
          row={row.original}
          idAccessor="accSetupCategoryId"
          onView={onAccountSetupCategorySelect}
          onEdit={onEditAccountSetupCategory}
          onDelete={onDeleteAccountSetupCategory}
        />
      ),
    },
    {
  ...
  const handleSearch = (query: string) => {
    ...
    if (data && data.length > 0) {
      ...
      const newFilters: IAccountSetupCategoryFilter = {
        search: query,
  ...
  useEffect(() => {
    if (!data?.length && onFilterChange) {
      const filters: IAccountSetupCategoryFilter = {
        search: searchQuery,
        sortOrder: sorting[0]?.desc ? "desc" : "asc",
      }
  ...
  return (
    <>
      <TableHeader
        ...
        onRefresh={onRefresh}
        onCreate={onCreateAccountSetupCategory}
