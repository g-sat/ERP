...
interface AccountSetupDtTableProps {
  ...
  isLoading?: boolean
  onAccountSetupDtSelect?: (accountSetupDt: IAccountSetupDt | null) => void
  onDeleteAccountSetupDt?: (accountSetupDtId: string) => void
  onEditAccountSetupDt?: (accountSetupDt: IAccountSetupDt) => void
  onCreateAccountSetupDt?: () => void
  onRefresh?: () => void
  onFilterChange?: (filters: IAccountSetupDtFilter) => void
  moduleId?: number
...
export function AccountSetupDtTable({
  data,
  isLoading = false,
  onAccountSetupDtSelect,
  onDeleteAccountSetupDt,
  onEditAccountSetupDt,
  ...
  const columns: ColumnDef<IAccountSetupDt>[] = [
    {
      ...
      cell: ({ row }) => (
        <TableActions
          ...
          onView={onAccountSetupDtSelect}
          onEdit={onEditAccountSetupDt}
          onDelete={onDeleteAccountSetupDt}
        />
      ),
    },
  ...
  const handleSearch = (query: string) => {
    ...
    if (data && data.length > 0) {
      ...
      const newFilters: IAccountSetupDtFilter = {
        search: query,
  ...
  useEffect(() => {
    if (!data?.length && onFilterChange) {
      const filters: IAccountSetupDtFilter = {
        search: searchQuery,
        sortOrder: sorting[0]?.desc ? "desc" : "asc",
      }
  ...
  return (
    <>
      <TableHeader
        ...
        onRefresh={onRefresh}
        onCreate={onCreateAccountSetupDt}
        columns={table.getAllLeafColumns()}
        data={data}
        tableName={TableName.account_setup_dt}
        hideCreateButton={false}
        moduleId={moduleId || 1}
        transactionId={transactionId || MasterTransactionId.account_setup_dt}
      />
                  ...
                  {virtualRows.length === 0 ? (
                    <TableRow>
                      ...
                      >
                        {isLoading
                          ? "Loading..."
                          : "No account setup details found."}
                      </TableCell>
                    </TableRow>
                  ) : (
                    <>
