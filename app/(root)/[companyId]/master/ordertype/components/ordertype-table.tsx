...
import { IOrderType, IOrderTypeFilter } from "@/interfaces/ordertype"
...
interface OrderTypeTableProps {
  data: IOrderType[]
  isLoading?: boolean
  onOrderTypeSelect?: (ordertype: IOrderType | undefined) => void
  onDeleteOrderType?: (orderTypeId: string) => void
  onEditOrderType?: (ordertype: IOrderType) => void
  onCreateOrderType?: () => void
  onRefresh?: () => void
  ...
  const columns: ColumnDef<IOrderType>[] = [
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <TableActions
            row={ordertype}
            idAccessor="orderTypeId"
            onView={onOrderTypeSelect}
            onEdit={onEditOrderType}
            onDelete={onDeleteOrderType}
          />
        )
      },
    },
    {
      ...
      header: "Code",
      cell: ({ row }) => (
        <div className="font-medium">{row.getValue("ordertypeCode")}</div>
      ),
      size: 120,
      minSize: 50,
      maxSize: 150,
...

    {
      ...
      cell: ({ row }) => (
        <div className="font-medium">
          {row.getValue("ordertypeCategoryName")}
        </div>
      ),
      size: 120,
      minSize: 50,
