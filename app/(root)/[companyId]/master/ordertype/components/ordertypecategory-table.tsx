...
interface OrderTypeCategoryTableProps {
  ...
  isLoading?: boolean
  onOrderTypeCategorySelect?: (
    ordertypedtcategory: IOrderTypeCategory | undefined
  ) => void
  onDeleteOrderTypeCategory?: (ordertypeId: string) => void
  onEditOrderTypeCategory?: (ordertypedtcategory: IOrderTypeCategory) => void
  onCreateOrderTypeCategory?: () => void
  onRefresh?: () => void
...
export function OrderTypeCategoryTable({
  ...
  const columns: ColumnDef<IOrderTypeCategory>[] = [
    {
      ...
      maxSize: 150,
      cell: ({ row }) => {
        const ordertypecategory = row.original
        return (
          <TableActions
            row={ordertypecategory}
            idAccessor="orderTypeCategoryId"
            onView={onOrderTypeCategorySelect}
            onEdit={onEditOrderTypeCategory}
            onDelete={onDeleteOrderTypeCategory}
          />
        )
      },
    },
  ...
  const handleSearch = (query: string) => {
    ...
    if (data && data.length > 0) {
      ...
      const newFilters: IOrderTypeFilter = {
        search: query,
  ...
  useEffect(() => {
    if (!data?.length && onFilterChange) {
      const filters: IOrderTypeFilter = {
        search: searchQuery,
        sortOrder: sorting[0]?.desc ? "desc" : "asc",
      }
  ...
  return (
    <div>
      <TableHeader
        ...
        onRefresh={onRefresh}
        onCreate={onCreateOrderTypeCategory}
