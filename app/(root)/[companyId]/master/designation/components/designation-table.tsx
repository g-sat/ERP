...
interface DesignationsTableProps {
  data: IDesignation[]
  isLoading?: boolean
  onDesignationSelect?: (designation: IDesignation | null) => void
...
export function DesignationsTable({
  ...
  const columns: ColumnDef<IDesignation>[] = [
    {
      ...
      cell: ({ row }) => (
        <TableActions
          row={row.original}
          idAccessor="designationId"
          onView={onDesignationSelect}
    ...
    },
    {
      ...
      header: "Code",
      cell: ({ row }) => (
        <div className="font-medium">{row.getValue("designationCode")}</div>
      ),
      size: 120,
      minSize: 50,
      maxSize: 150,
  ...
  const handleSearch = (query: string) => {
    ...
    if (data && data.length > 0) {
      ...
      const newFilters: IDesignationFilter = {
        search: query,
  ...
  useEffect(() => {
    if (!data?.length && onFilterChange) {
      const filters: IDesignationFilter = {
        search: searchQuery,
        sortOrder: sorting[0]?.desc ? "desc" : "asc",
      }
