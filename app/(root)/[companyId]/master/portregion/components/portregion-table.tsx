...
interface PortRegionsTableProps {
  data: IPortRegion[]
  isLoading?: boolean
  onPortRegionSelect?: (portregion: IPortRegion | null) => void
  onDeletePortRegion?: (portRegionId: string) => void
  onEditPortRegion?: (portregion: IPortRegion) => void
  onCreatePortRegion?: () => void
  onRefresh?: () => void
  onFilterChange?: (filters: IPortRegionFilter) => void
  moduleId?: number
...
export function PortRegionsTable({
  ...
  const columns: ColumnDef<IPortRegion>[] = [
    {
      ...
      cell: ({ row }) => (
        <TableActions
          row={row.original}
          idAccessor="portRegionId"
          onView={onPortRegionSelect}
          onEdit={onEditPortRegion}
          onDelete={onDeletePortRegion}
        />
      ),
    },
    {
      ...
      header: "Code",
      cell: ({ row }) => (
        <div className="font-medium">{row.getValue("portRegionCode")}</div>
      ),
      size: 120,
      minSize: 50,
      maxSize: 150,
  ...
  const handleSearch = (query: string) => {
    ...
    if (data && data.length > 0) {
      ...
      const newFilters: IPortRegionFilter = {
        search: query,
  ...
  useEffect(() => {
    if (!data?.length && onFilterChange) {
      const filters: IPortRegionFilter = {
        search: searchQuery,
        sortOrder: sorting[0]?.desc ? "desc" : "asc",
      }
  ...
  return (
    <div>
      <TableHeader
        ...
        onRefresh={onRefresh}
        onCreate={onCreatePortRegion}
        columns={table.getAllLeafColumns()}
        data={data}
        tableName={TableName.port_region}
        hideCreateButton={false}
        moduleId={moduleId || 1}
        transactionId={transactionId || MasterTransactionId.port_region}
      />
