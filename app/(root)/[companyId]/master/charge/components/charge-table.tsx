...
interface ChargesTableProps {
  data: ICharge[]
  isLoading?: boolean
  onChargeSelect?: (charge: ICharge | undefined) => void
  onDeleteCharge?: (chargeId: string) => void
  onEditCharge?: (charge: ICharge) => void
  onCreateCharge?: () => void
  onRefresh?: () => void
...
export function ChargesTable({
  ...
  const columns: ColumnDef<ICharge>[] = [
    ...
    },
    {
      ...
      header: "Code",
      cell: ({ row }) => (
        <div className="font-medium">{row.getValue("chargeCode")}</div>
      ),
      size: 120,
      minSize: 50,
      maxSize: 150,
    ...
    },
    {
      accessorKey: "glId",
      header: "GL ID",
      cell: ({ row }) => <div>{row.getValue("glId") || "—"}</div>,
      size: 120,
      minSize: 50,
      maxSize: 150,
    },
  ...
  const handleSearch = (query: string) => {
    ...
    if (data && data.length > 0) {
      ...
      const newFilters: IChargeFilter = {
        search: query,
