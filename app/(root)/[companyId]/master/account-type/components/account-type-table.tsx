...
interface AccountTypesTableProps {
  data: IAccountType[]
  isLoading?: boolean
  onAccountTypeSelect?: (accountType: IAccountType | undefined) => void
  onDeleteAccountType?: (accountTypeId: string) => void
  onEditAccountType?: (accountType: IAccountType) => void
  onCreateAccountType?: () => void
  onRefresh?: () => void
  onFilterChange?: (filters: IAccountTypeFilter) => void
  moduleId?: number
...
export function AccountTypesTable({
  ...
  const handleSearch = (query: string) => {
    ...
    if (data && data.length > 0) {
      ...
      const newFilters: IAccountTypeFilter = {
        search: query,
  ...
  useEffect(() => {
    if (!data?.length && onFilterChange) {
      const filters: IAccountTypeFilter = {
        search: searchQuery,
        sortOrder: sorting[0]?.desc ? "desc" : "asc",
      }
  ...
  return (
    <div>
      <TableHeader
        ...
        moduleId={moduleId || 1}
        transactionId={transactionId || MasterTransactionId.account_type}
      />
