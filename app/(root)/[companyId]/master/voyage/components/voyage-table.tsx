...
interface VoyagesTableProps {
  data: IVoyage[]
  isLoading?: boolean
  onVoyageSelect?: (voyage: IVoyage | null) => void
  onDeleteVoyage?: (voyageId: string) => void
  onEditVoyage?: (voyage: IVoyage) => void
  onCreateVoyage?: () => void
  onRefresh?: () => void
...
export function VoyagesTable({
  ...
  const columns: ColumnDef<IVoyage>[] = [
    {
      ...
      cell: ({ row }) => (
        <TableActions
          row={row.original}
          idAccessor="voyageId"
          onView={onVoyageSelect}
          onEdit={onEditVoyage}
          onDelete={onDeleteVoyage}
        />
      ),
    },
    {
      ...
      header: "Voyage No",
      cell: ({ row }) => (
        <div className="font-medium">{row.getValue("voyageNo")}</div>
      ),
      size: 200,
      minSize: 50,
      maxSize: 300,
    ...
    },
    {
      ...
      header: "Reference No",
      cell: ({ row }) => (
        <div className="font-medium">{row.getValue("referenceNo")}</div>
      ),
      size: 120,
      minSize: 50,
      maxSize: 150,
  ...
  const handleSearch = (query: string) => {
    ...
    if (data && data.length > 0) {
      ...
      const newFilters: IVoyageFilter = {
        search: query,
