...
interface DepartmentsTableProps {
  data: IDepartment[]
  isLoading?: boolean
  onDepartmentSelect?: (department: IDepartment | null) => void
  onDeleteDepartment?: (departmentId: string) => void
  onEditDepartment?: (department: IDepartment) => void
  onCreateDepartment?: () => void
  onRefresh?: () => void
  onFilterChange?: (filters: IDepartmentFilter) => void
  moduleId?: number
...
export function DepartmentsTable({
  ...
  const columns: ColumnDef<IDepartment>[] = [
    {
      ...
      cell: ({ row }) => (
        <TableActions
          row={row.original}
          idAccessor="departmentId"
          onView={onDepartmentSelect}
    ...
    },
    {
      ...
      header: "Code",
      cell: ({ row }) => (
        <div className="font-medium">{row.getValue("departmentCode")}</div>
      ),
      size: 120,
      minSize: 50,
      maxSize: 150,
  ...
  const handleSearch = (query: string) => {
    ...
    if (data && data.length > 0) {
      ...
      const newFilters: IDepartmentFilter = {
        search: query,
  ...
  return (
    <div>
      <TableHeader
        ...
        transactionId={transactionId || MasterTransactionId.department}
        companyId={companyId}
      />
