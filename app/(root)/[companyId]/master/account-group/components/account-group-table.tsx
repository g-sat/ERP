...
interface AccountGroupsTableProps {
  data: IAccountGroup[]
  isLoading?: boolean
  onAccountGroupSelect?: (accountGroup: IAccountGroup | undefined) => void
  onDeleteAccountGroup?: (accountGroupId: string) => void
  onEditAccountGroup?: (accountGroup: IAccountGroup) => void
  ...
  const columns: ColumnDef<IAccountGroup>[] = [
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <TableActions
            row={accountGroup}
            idAccessor="accGroupId"
            onView={onAccountGroupSelect}
            onEdit={onEditAccountGroup}
    ...
    },
    {
      ...
      header: "Code",
      cell: ({ row }) => (
        <div className="font-medium">{row.getValue("accGroupCode")}</div>
      ),
      size: 120,
      minSize: 50,
      maxSize: 150,
