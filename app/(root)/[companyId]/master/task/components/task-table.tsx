...
interface TasksTableProps {
  data: ITask[]
  isLoading?: boolean
  onTaskSelect?: (task: ITask | null) => void
...
export function TasksTable({
  ...
  const columns: ColumnDef<ITask>[] = [
    ...
    },
    {
      ...
      header: "Code",
      cell: ({ row }) => (
        <div className="font-medium">{row.getValue("taskCode")}</div>
      ),
      size: 120,
      minSize: 50,
      maxSize: 150,
    ...
    {
      accessorKey: "taskOrder",
      header: "Task Order",
      cell: ({ row }) => <div>{row.getValue("taskOrder") || "—"}</div>,
      size: 120,
      minSize: 50,
      maxSize: 150,
    },
